...
const METER_VALUE_INTERVAL = 30000; // 30 seconds

const DEFAULT_AUTOCHARGE_VID_TAG = (process.env.AUTOCHARGE_VID_TAG || "").trim();

// Hardcoded User Array - Provide one or more users here
const ALL_USERS = [
    "58e54a38-33aa-4fca-a4df-05bd15825d39+916369070422+CUSTOMER", // Default user
    "58e54a38-33aa-4fca-a4df-05bd15825d39+919486939888+CUSTOMER"
];

// parsing the --limit command-line argument so the simulation only runs a subset of charge points.
// takes everything after the script name
let limit = null;
...
// Read config.json to build TARGET_CONFIG dynamically(for each charge point, we create a simulator instance)
...
// Allocate users deterministically (round-robin) so multiple sessions use different users.
let userCursor = 0;
function getNextUser() {
    // Select the next user for a simulator instance (round-robin allocation).
    if (!ALL_USERS.length) return "";
    const user = ALL_USERS[userCursor % ALL_USERS.length];
    userCursor++;
    return user;
}

let activeSimulatorsCount = 0;

// Stop Coordinator: ensures we stop in sets of 2
class StopCoordinator {
    constructor() {
        // Create a queue for coordinating stop calls in batches.
        this.queue = [];
    }

    async waitForBatch() {
        // Wait until at least 2 simulators are ready to stop, then release them together.
    ...
    checkQueue() {
        // Resolve queued stop waits in groups of 2; flush remaining when no sims are running.
        // If we have at least 2 sessions waiting to stop, pop 2 and let them proceed
        if (this.queue.length >= 2) {
            console.log(`[Coordinator] Flushing 2 sessions together...`);
            const toResolve = this.queue.splice(0, 2);
            for (const resolve of toResolve) {
                resolve();
            }
        }
        // If the only active simulators left are already in the stop queue, let them finish
        else if (this.queue.length > 0 && this.queue.length === activeSimulatorsCount) {
            console.log(`[Coordinator] Flushing remaining ${this.queue.length} sessions (no more simulators running)...`);
            const toResolve = this.queue.splice(0, this.queue.length);
            for (const resolve of toResolve) {
                resolve();
            }
        }
...
function parseCliArgValue(name) {
    // Read a CLI argument value of the form `--name value` or `--name=value` (for example --mode=autocharge or --vid=VID:3QW1234567890)
...
function recordResult(cpId, result, socStart, socEnd, error = null) {
    // Persist a finished-session outcome into the in-memory report.
...
function printReport() {
    // Print a human-readable summary and also save it as report.json.
...
class ChargePointSimulator {
    constructor(cpId, connectorId, limitWh, vidTag = '') {
        // Initialize per-connector simulator state (OCPP transaction, SoC/Energy progression, etc.).
        this.cpId = cpId;
    ...
    nextId() {
        // Produce an incrementing message id for OCPP CALL/RESULT correlation.
        return (this.nextMessageId++).toString();
    }

    sendCall(action, payload) {
        // Send an OCPP CALL and resolve when the matching CALLRESULT arrives.
    ...
    async sendStatus(status) {
        // Send OCPP StatusNotification for the given connector status.
    ...
    async sendAuthorize(idTag) {
        // Send an OCPP Authorize request for the provided idTag.
        this.log(`<- [AuthorizeRequest] idTag=${idTag}`);
        return await this.sendCall("Authorize", { idTag });
    }

    releaseUser() {
        // Release the assigned user id from this simulator instance.
        this.userId = null;
    }

    markDone(result, errorMsg = null) {
        // Finalize the simulator: mark as finished, record result, and update global counters.
        if (this.finished) return;
        this.finished = true;
        this.releaseUser();
        recordResult(this.cpId, result, this.socStart.toFixed(1), this.socValue.toFixed(1), errorMsg);

        activeSimulatorsCount--;
        stopCoordinator.checkQueue(); // In case this was the last active one
    }

    async apiCall(endpoint, method = 'GET', body = null, attempt = 1) {
        // Perform a REST API call with exponential backoff retry.
        ...
        try {
            const headers = {
                ...
                "origin": "https://steam-ev-dev.d2bmp177mlllvs.amplifyapp.com/",
                "priority": "u=1, i",
                "referer": "https://steam-ev-dev.d2bmp177mlllvs.amplifyapp.com//",
    ...
    async triggerRemoteStart() {
        // Assign a user id and ask the backend to create/trigger a charging session.
        this.userId = getNextUser();
        this.log(`Assigned User ID: ${this.userId}. Triggering Remote Start for Connector ${this.connectorId}...`);
        try {
    ...
    async runStartupSequence() {
        // Drive the initial OCPP flow (Available/Preparing/Authorize) and then trigger RemoteStart.
        // Default flow: Available -> (after ack) RemoteStart API
    ...
    async sendSingleMeterValue(isFinal = false) {
        // Send one OCPP MeterValues message with current energy/SoC/voltage samples.
    ...
    async fetchAndTriggerStop() {
        // Call the local stop service, send OCPP StopTransaction to CSMS, and end the session.
    ...
    async sendMeterValueSequence() {
        // Progress energy/SoC over multiple MeterValues messages and then coordinate the stop phase.
        this.log(`Starting 2x sequence. Limit: ${this.limitWh} Wh`);

        const totalIncrement = Math.max(0, this.limitWh - 1000);
        const stepIncrement = Math.floor(totalIncrement / 2);
        ...
        if (!this.finished) {
            this.log("Sequence done. Entering stop queue, awaiting paired stop...");
            await stopCoordinator.waitForBatch();

            this.log("Batch paired. Triggering stop flow in 2s...");
            setTimeout(() => this.fetchAndTriggerStop(), 2000);
        }
    }

    start() {
        // Open the WebSocket to CSMS and orchestrate the OCPP call/response lifecycle.
        activeSimulatorsCount++;
        ...
        this.ws.addEventListener('close', () => {
            ...
            if (!this.finished && this.transactionId) {
                // If disconnected prematurely, attempt to clean up
                this.markDone('FAILURE', 'Premature WS close');
            }
        });
    ...
    async handleConnectError(msg) {
        // Retry startup on connection problems; if exhausted, trigger stop cleanup and record failure.
        if (this.finished) return;
        const MAX_CONNECT_ATTEMPTS = 3;
...
async function run() {
    // Launch simulators for all targeted charge points while respecting a global concurrency cap.
    ...
    console.log(`[Main] Launching custom simulation for ${totalToRun} targeted connectors...`);
    console.log(`[Main] Users defined: ${ALL_USERS.length}`);

    for (let i = 0; i < totalToRun; i++) {
        // Concurrency limit
        while (activeSims.size >= 50) {
        ...
        sim.markDone = function (result, errorMsg) {
            // Wrap `markDone` so we also remove this simulator from the active set.
